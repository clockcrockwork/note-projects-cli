...
const PUBLIC_COMMANDS = COMMANDS.map((argv) => argv.join(' '))
...
export function isSafePatreonRepositoryPath(path) {
...
export function planPatreonExport(listing) {
  ...
  for (const entry of listing.tree) {
    ...
    if (!isSafePatreonRepositoryPath(path)) throw new Error('SOURCE_EXPORT_PATH_UNSAFE')
    ...
    if (isExportablePatreonPath(path)) {
      if (!/^[0-9a-f]{40}$/.test(entry?.sha ?? '')) {
        throw new Error('SOURCE_EXPORT_BLOB_SHA_INVALID')
      }
  ...
  for (const required of ['package.json', 'package-lock.json']) {
    if (!include.some((entry) => entry.path === required)) {
      throw new Error(required === 'package.json'
        ? 'SOURCE_EXPORT_PACKAGE_JSON_MISSING'
        : 'SOURCE_EXPORT_PACKAGE_LOCK_MISSING')
    }
  }
...
async function main() {
  ...
  let sourceSha = null
  const root = resolve(process.env.RUNNER_TEMP || tmpdir(), `patreon-ci-${process.pid}`)
  ...
  try {
    ...
    const exportPlan = planPatreonExport(listing)
    ...
    for (const argv of COMMANDS) {
      const run = runCaptured(isolatedDockerArgs({ image, sourceRoot, argv }), {
        timeout: 20 * 60_000,
        env: childEnv(),
      })
      if (!run.ok) {
        const diagnostic = {
          ...
          'public-view:build': 'PATREON_PUBLIC_VIEW_BUILD_FAILED',
        }[argv.slice(2).join(' ')] ?? 'PATREON_VERIFY_FAILED'
    ...
    if (accessToken) {
      try {
        ...
        // Best-effort revocation only. No private source command executes
        // while the installation token remains available.
      }
    }
    delete process.env.NPR_APP_PRIVATE_KEY
    await rm(root, { recursive: true, force: true })
